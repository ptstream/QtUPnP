...
public :
   ...
   * If nt is empty, upnp:rootdevice is used.
   *
   * The aurgument must be:
   * \li ssdp:all Search for all devices and services.
   * \li upnp:rootdevice Search for root devices only.
   * \li uuid:device-UUID Search for a particular device.
   * \li device-UUID specified by UPnP vendor.
   * \li urn:schemas-upnp-org:device:deviceType:ver Search for any device of this type where deviceType and
   * ver are defined by the UPnP Forum working committee.
   * \li urn:schemas-upnp-org:service:serviceType:ver Search for any service of this type where serviceType and
   * ver are defined by the UPnP Forum working committee.
   * \li urn:domain-name:device:deviceType:ver Search for any device of this typewhere domain-name (a Vendor Domain Name),
   * deviceType and ver are defined by the UPnP vendor and ver specifies the highest specifies the highest supported
   * version of the device type.
   * \li urn:domain-name:service:serviceType:ver Search for any service of this type.
   * Where domain-name (a Vendor Domain Name), serviceType and ver are defined by the UPnP vendor and
   * ver specifies the highest specifies the highest supported version of the service type.
   *
   * e.g. you can launch the delivery for a device uuid by calling:
   * \code
   * discover ("uuid:f699669e-d287-4291-9ed5-a91ecb3a90b8");
   * \endcode
   * \return true if success, false for error.
   */
  ...
  */
  ...
  /*! Returns the device uuid and the url for UPnP description for device.
   * The uuid is the first element of the pair.
   * The url is the second element of the pair.
   *
   * You can store these two values in a file for the next startup.
   * Before call discovery your can read the file and call extractDevices.
   * If the uuid, ip address and port are not changed, this speed up greatly the discovery.
   * Moreover, if you know the number of devices reached, you can skip discovry.
   *
   ...
   * \li The device is off.
   * \li The DHCP has changed the ip address.
   * \li The port has changed.
   * \li The uuid has changed (very rare case).
   * In these cases, the device is not inserted in the device map.
   *
   * \param pairs: The list of uuid, url pair.
