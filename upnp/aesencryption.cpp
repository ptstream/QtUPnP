 ...
 * Static Functions
 * */
QByteArray CAESEncryption::Crypt(CAESEncryption::AES level, CAESEncryption::MODE mode, const QByteArray rawText, const QByteArray key, const QByteArray iv)
{
    return CAESEncryption(level, mode).encode(rawText, key, iv);
}

QByteArray CAESEncryption::Decrypt(CAESEncryption::AES level, CAESEncryption::MODE mode, const QByteArray rawText, const QByteArray key, const QByteArray iv)
{
     return CAESEncryption(level, mode).decode(rawText, key, iv);
}

QByteArray CAESEncryption::ExpandKey(CAESEncryption::AES level, CAESEncryption::MODE mode, const QByteArray key)
{
     return CAESEncryption(level, mode).expandKey(key);
}
/*
...
CAESEncryption::CAESEncryption(CAESEncryption::AES level, CAESEncryption::MODE mode)
...
QByteArray CAESEncryption::expandKey(const QByteArray key)
{
  int i, k;
...
void CAESEncryption::addRoundKey(const quint8 round, const QByteArray expKey)
{
...
//optimized!!
void CAESEncryption::mixColumns()
{
  QByteArray::iterator it = m_state->begin();
  quint8 tmp, tm, t;
...
void CAESEncryption::invMixColumns()
{
  QByteArray::iterator it = m_state->begin();
  quint8 a,b,c,d;
  for(int i = 0; i < 16; i+=4){
...
// The SubBytes Function Substitutes the values in the
// state matrix with values in an S-box.
void CAESEncryption::invSubBytes()
{
    QByteArray::iterator it = m_state->begin();
    for(int i = 0; i < 16; ++i)
...
void CAESEncryption::invShiftRows()
{
    QByteArray::iterator it = m_state->begin();
    uint8_t temp;
...
QByteArray CAESEncryption::byteXor(const QByteArray a, const QByteArray b)
{
...
QByteArray CAESEncryption::cipher(const QByteArray expKey, const QByteArray in)
{
...
QByteArray CAESEncryption::invCipher(const QByteArray expKey, const QByteArray in)
{
...
QByteArray CAESEncryption::encode(const QByteArray rawText, const QByteArray key, const QByteArray iv)
{
...
QByteArray CAESEncryption::decode(const QByteArray rawText, const QByteArray key, const QByteArray iv)
{
    if (m_mode >= CBC && (iv.isNull() || iv.size() != m_blocklen))
       return QByteArray();
